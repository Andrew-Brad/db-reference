-- File originally sourced from: https://github.com/neondatabase-labs/postgres-sample-dbs
-- If using a psql client: psql -d "postgres://<user>:<password>@<hostname>/periodic_table" -f periodic_table.sql
-- If psql is not available, but running postgres on docker: docker exec -i <containerId> psql -U <user> < periodic_table.sql
