-- File originally sourced from: https://github.com/pthom/northwind_psql
-- If using a psql client: psql -d "postgres://<user>:<password>@<hostname>/northwind" -f northwind.sql
-- Create a db first: "northwind"
-- If psql is not available, but running postgres on docker: docker exec -i <containerId> psql --username=<user> --dbname=usda < usda.sql
